...
4. REST+GraphQL APIs with OpenAPI & GraphQL SDL docs.  
5. Containerised deployment (Docker) with CI/CD to staging & prod (Kubernetes).  
...
```
[ Browser ] ⇆ [ Next.js SSR SPA ] —gRPC/REST→ [ NestJS micro‑services ] —SQL→ [ PostgreSQL (schema‑per‑tenant) ]
                                         ↓                                   ↘︎
                                   [ Redis cache ]                        [ MinIO object storage ]
```
* **Frontend:** Next.js 14, React 18, Tailwind CSS, TanStack Query  
* **Backend:** NestJS 10, TypeScript, Prisma ORM  
* **Auth:** Keycloak / Auth0; JWT + Refresh; optional SSO (SAML/OIDC)  
* **Messaging:** NATS (event bus) for async tasks  
* **CI/CD:** GitHub Actions → Docker → Helm → EKS  

## 9. Data Model (high‑level)  
* `core.tenant` ←1‑M→ `core.user`  
* `school.student` ←M‑M→ `school.class`  
* `finance.fee_invoice` → `finance.payment` (polymorphic: cash, Stripe, bank)  
...
_To be generated by the architecture agent using Prisma schema._
