...
const schema = yup.object().shape({
  ...
  homeroomTeacherId: yup
    .string()
    .uuid('Must be a valid UUID if provided')
    .nullable()
    .optional(),
})
...
const ClassForm: React.FC<ClassFormProps> = ({
  ...
  const handleFormSubmit: SubmitHandler<CreateClassDto | UpdateClassDto> = (
    data,
  ) => {
  ...
  return (
    <Card className='w-full max-w-lg'>
      <form onSubmit={handleSubmit(handleFormSubmit)} className='space-y-4'>
        ...
        <div>
          <label htmlFor='homeroomTeacherId' className='block mb-1 font-medium'>
            Homeroom Teacher ID (Optional UUID)
          </label>
          <Input
