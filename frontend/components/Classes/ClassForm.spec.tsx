...
describe('ClassForm', () => {
  ...
  it('renders correctly for editing an existing class and pre-fills data', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByLabelText(/level\/grade/i)).toHaveValue(
        mockClass.level,
      )
      expect(screen.getByLabelText(/homeroom teacher id/i)).toHaveValue(
        mockClass.homeroomTeacherId,
      )
    })
    expect(
      screen.getByRole('button', { name: /update class/i }),
    ).toBeInTheDocument()
  })
  ...
  it('calls onSubmit with form data when creating a class', async () => {
    ...
    fireEvent.change(screen.getByLabelText(/level\/grade/i), {
      target: { value: testData.level },
    })
    fireEvent.change(screen.getByLabelText(/homeroom teacher id/i), {
      target: { value: testData.homeroomTeacherId },
    })
