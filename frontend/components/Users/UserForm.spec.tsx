...
jest.mock('../ui/PasswordToggle', () => (props: any) => (
  <input
    type='password'
...
describe('UserForm', () => {
  ...
  it('renders correctly for editing an existing user and pre-fills data (excluding password)', async () => {
    render(
      <UserForm user={mockUser} onSubmit={mockOnSubmit} isSubmitting={false} />,
    )
    ...
    await waitFor(() => {
      ...
      if (!mockUser.roles.includes(UserRole.TEACHER)) {
        // Example of a role not assigned
        const teacherCheckbox = screen.getByLabelText(
          new RegExp(UserRole.TEACHER, 'i'),
        ) as HTMLInputElement
        expect(teacherCheckbox.checked).toBe(false)
      }

      expect(
        (screen.getByLabelText(/active user/i) as HTMLInputElement).checked,
      ).toBe(mockUser.isActive)
    })
    expect(
  ...
  it('calls onSubmit with form data when creating a user', async () => {
    ...
    const defaultUserRoleCheckbox = screen.getByLabelText(
      new RegExp(UserRole.USER, 'i'),
    ) as HTMLInputElement
    ...
    if (activeCheckbox.checked) {
      // Default is true, so uncheck it for testData.isActive = false
    ...
    await waitFor(() => {
      ...
      expect(submittedData.roles).toEqual(
        expect.arrayContaining(testData.roles),
      )
