...
const schema = yup.object().shape({
  ...
  password: yup.string().when('$isEditing', {
    // $isEditing context passed from useForm
    ...
    otherwise: (s) =>
      s.min(8, 'Password must be at least 8 characters').optional().nullable(), // Optional for edit
  }),
...
const UserForm: React.FC<UserFormProps> = ({
  ...
  const handleFormSubmit: SubmitHandler<CreateUserDto | UpdateUserDto> = (
    data,
  ) => {
    const submittedData = { ...data }
    if (isEditing && !submittedData.password) {
      // If password field is empty during edit, don't send it
