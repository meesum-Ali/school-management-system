...
describe('StudentList', () => {
  ...
  it('renders correctly with a list of students', () => {
    ...
    expect(
      screen.getByText(
        new Date(mockStudents[0].dateOfBirth).toLocaleDateString(),
      ),
    ).toBeInTheDocument()
    expect(
      screen.getByText(
        new Date(mockStudents[0].createdAt).toLocaleDateString(),
      ),
    ).toBeInTheDocument()
    ...
    expect(
      screen.getByText(
        new Date(mockStudents[1].dateOfBirth).toLocaleDateString(),
      ),
    ).toBeInTheDocument()
    expect(
      screen.getByText(
        new Date(mockStudents[1].createdAt).toLocaleDateString(),
      ),
    ).toBeInTheDocument()
    ...
    expect(
      screen.getByRole('link', { name: /create student/i }),
    ).toBeInTheDocument()
  })
  ...
  it('Edit button has correct link', () => {
    ...
    expect(editLinks[0]).toHaveAttribute(
      'href',
      `/admin/students/${mockStudents[0].id}`,
    )
    expect(editLinks[1]).toHaveAttribute(
      'href',
      `/admin/students/${mockStudents[1].id}`,
    )
  })
})
