...
describe('StudentForm', () => {
  ...
  it('renders correctly for creating a new student', () => {
    ...
    expect(
      screen.getByRole('button', { name: /create student/i }),
    ).toBeInTheDocument()
  })
  ...
  it('renders correctly for editing an existing student and pre-fills data', () => {
    ;(jest.requireMock('react-hook-form') as any).useForm = () => ({
      ...
      formState: { errors: {} },
      defaultValues: {
        // Simulate defaultValues behavior from react-hook-form
        ...mockStudent,
        dateOfBirth: new Date(mockStudent.dateOfBirth)
          .toISOString()
          .split('T')[0],
      },
    })
    ...
    expect(screen.getByLabelText(/date of birth/i)).toHaveValue(
      new Date(mockStudent.dateOfBirth).toISOString().split('T')[0],
    )
  ...
  it('calls onSubmit with form data when creating a student', async () => {
    ...
    fireEvent.change(screen.getByLabelText(/student id/i), {
      target: { value: testData.studentId },
    })
