...
describe('SubjectList', () => {
  const mockSubjects: Subject[] = [
    ...
    },
    {
      ...
      code: 'CHEM202',
      description: null,
      classes: [{ id: 'c1', name: 'Chem Class', level: '2', createdAt: '', updatedAt: '' }],
      createdAt: new Date().toISOString(),
      updatedAt: new Date().toISOString()
    },
  ];
  ...
  it('renders correctly with a list of subjects', () => {
    ...
    expect(screen.getByText('Multivariable calculus and theorems.')).toBeInTheDocument();
    ...
    const editButtons = screen.getAllByRole('button', { name: /edit \/ view classes/i });
  ...
  it('Edit button has correct link', () => {
    render(<SubjectList subjects={mockSubjects} onDelete={mockOnDelete} />);
    const editLinks = screen.getAllByRole('link', { name: /edit \/ view classes/i });
    expect(editLinks[0]).toHaveAttribute('href', `/admin/subjects/${mockSubjects[0].id}`);
  });
});
