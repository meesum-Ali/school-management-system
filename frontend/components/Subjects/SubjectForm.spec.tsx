...
jest.mock('../ui/Textarea', () => (props: any) => <textarea data-testid="description-textarea" {...props} />);
...
describe('SubjectForm', () => {
  ...
  it('renders correctly for editing an existing subject and pre-fills data', async () => {
    render(<SubjectForm initialData={mockSubject} onSubmit={mockOnSubmit} isSubmitting={false} />);

    await waitFor(() => {
        expect(screen.getByLabelText(/subject name/i)).toHaveValue(mockSubject.name);
        expect(screen.getByLabelText(/code \(optional\)/i)).toHaveValue(mockSubject.code);
        expect(screen.getByLabelText(/description \(optional\)/i)).toHaveValue(mockSubject.description);
    });
  ...
  it('calls onSubmit with form data when creating a subject', async () => {
    ...
    fireEvent.change(screen.getByLabelText(/subject name/i), { target: { value: testData.name } });
    fireEvent.change(screen.getByLabelText(/code \(optional\)/i), { target: { value: testData.code } });
    fireEvent.change(screen.getByLabelText(/description \(optional\)/i), { target: { value: testData.description } });
