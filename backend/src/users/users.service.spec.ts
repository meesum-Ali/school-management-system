...
describe('UsersService', () => {
  ...
  const userDto = new UserDto({ ...mockUserEntity });
  ...
  beforeEach(async () => {
    ...
    (bcrypt.hash as jest.Mock).mockClear(); // Clear mock call counts for bcrypt.hash
  });
  ...
  describe('create', () => {
    ...
    it('should create and return a user dto if username and email are unique', async () => {
      ...
      const savedUserEntity = { ...new User(), ...createUserDto, id: 'new-uuid', password: 'hashedPasswordFromSave' };
      ...
      expect(repository.findOne).toHaveBeenCalledWith({ where: [{ username: createUserDto.username }, { email: createUserDto.email }] });
    ...
    it('should throw ConflictException if username already exists', async () => {
      repository.findOne.mockResolvedValue({ ...mockUserEntity, username: createUserDto.username });
      await expect(service.create(createUserDto)).rejects.toThrow(ConflictException);
    });

    it('should throw ConflictException if email already exists', async () => {
      repository.findOne.mockResolvedValue({ ...mockUserEntity, email: createUserDto.email });
      await expect(service.create(createUserDto)).rejects.toThrow(ConflictException);
    });

    it('should throw ConflictException on save if username constraint violated (race condition)', async () => {
      repository.findOne.mockResolvedValue(null);
      const queryFailedError = new QueryFailedError('query', [], { code: '23505', message: 'users_username_key' } as any);
      repository.save.mockRejectedValue(queryFailedError);
      await expect(service.create(createUserDto)).rejects.toThrow(new ConflictException('Username already exists.'));
    });

    it('should throw ConflictException on save if email constraint violated (race condition)', async () => {
        repository.findOne.mockResolvedValue(null);
        const queryFailedError = new QueryFailedError('query', [], { code: '23505', message: 'users_email_key' } as any);
        repository.save.mockRejectedValue(queryFailedError);
        await expect(service.create(createUserDto)).rejects.toThrow(new ConflictException('Email already exists.'));
    });
  });
  ...
  describe('update', () => {
    const updateUserDto: UpdateUserDto = { firstName: 'Updated', email: 'updated@example.com' };
    ...
    it('should update and return a user dto if found', async () => {
      ...
      repository.preload.mockResolvedValue({ ...mockUserEntity, ...updateUserDto });
      repository.save.mockResolvedValue({ ...mockUserEntity, ...updateUserDto });

      const result = await service.update('some-uuid', updateUserDto);

      expect(repository.preload).toHaveBeenCalledWith({ id: 'some-uuid', ...updateUserDto });
      expect(repository.save).toHaveBeenCalledWith({ ...mockUserEntity, ...updateUserDto });
    ...
    it('should throw NotFoundException if user to update is not found', async () => {
      ...
      await expect(service.update('bad-uuid', updateUserDto)).rejects.toThrow(NotFoundException);
    });

    it('should throw ConflictException if updated username already exists for another user', async () => {
        repository.findOne.mockResolvedValue({ ...mockUserEntity, id: 'another-uuid' }); // username exists on another user
        await expect(service.update('some-uuid', { username: 'testuser' })).rejects.toThrow(ConflictException);
    });

    it('should throw ConflictException if updated email already exists for another user', async () => {
        repository.findOne.mockResolvedValue({ ...mockUserEntity, id: 'another-uuid' }); // email exists on another user
        await expect(service.update('some-uuid', { email: 'test@example.com' })).rejects.toThrow(ConflictException);
    });
  });
