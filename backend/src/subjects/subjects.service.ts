...
export class SubjectsService {
  ...
  async create(createSubjectDto: CreateSubjectDto, schoolId: string): Promise<SubjectDto> {
    ...
    if (nameConflict) {
      throw new ConflictException(`Subject with name "${name}" already exists in this school.`);
    }
    if (code) {
      const codeConflict = await this.subjectsRepository.findOne({ where: { code, schoolId } });
      if (codeConflict) {
        throw new ConflictException(`Subject with code "${code}" already exists in this school.`);
      }
    }
  ...
  async update(id: string, updateSubjectDto: UpdateSubjectDto, schoolId: string): Promise<SubjectDto> {
    ...
    if (updateSubjectDto.hasOwnProperty('code')) { // Allow setting code to null
  ...
  async remove(id: string, schoolId: string): Promise<void> {
    ...
    if (result.affected === 0) {
      throw new NotFoundException(`Subject with ID "${id}" in this school not found or already deleted.`);
    }
  }
