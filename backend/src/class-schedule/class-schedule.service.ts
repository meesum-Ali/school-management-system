...
export class ClassScheduleService {
  ...
  async create(
    ...
    await this.classesService.findOne(
      createClassScheduleDto.classId,
      createClassScheduleDto.schoolId,
    );

    // Validate subject exists
    await this.subjectsService.findOne(
      createClassScheduleDto.subjectId,
      createClassScheduleDto.schoolId,
    );

    // Validate teacher exists if provided
    if (createClassScheduleDto.teacherId) {
      await this.teachersService.findOne(
  ...
  async update(
    ...
    if (updateClassScheduleDto.classId) {
      await this.classesService.findOne(
        updateClassScheduleDto.classId,
        schoolId,
      );
    }
    ...
    if (updateClassScheduleDto.subjectId) {
      await this.subjectsService.findOne(
        updateClassScheduleDto.subjectId,
        schoolId,
      );
    }
    ...
    if (updateClassScheduleDto.teacherId) {
      await this.teachersService.findOne(
        updateClassScheduleDto.teacherId,
        schoolId,
      );
    }
    ...
    const updated = this.classScheduleRepository.merge(
      schedule,
      updateClassScheduleDto,
    );
  ...
  private async checkForSchedulingConflicts(
    ...
    if (classConflict) {
      throw new ConflictException(
        'Class already has a scheduled class during this time slot',
      );
    }
    ...
    if (teacherId) {
      ...
      if (teacherConflict) {
        throw new ConflictException(
          'Teacher is already assigned to another class during this time slot',
        );
      }
    }
