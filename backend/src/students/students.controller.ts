...
export class StudentsController {
  ...
  @ApiResponse({
    ...
    description:
      'Conflict - Student ID or email already exists in this school.',
  })
  async create(
  ...
  @ApiResponse({
    status: HttpStatus.CONFLICT,
    description:
      'Conflict - Student ID or email already exists on another student.',
  })
  async update(
    @Param('id', ParseUUIDPipe) id: string,
  ...
  @ApiOperation({
    summary:
      "Assign or Unassign a student to/from a class in the admin's school (Admin only)",
  })
  @ApiParam({
    name: 'studentId',
    type: 'string',
  ...
  async assignClass(
    ...
    const schoolId = this.getSchoolIdFromRequest(req);
    return this.studentsService.assignStudentToClass(
      studentId,
