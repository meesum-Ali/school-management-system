...
describe('StudentsService', () => {
  ...
  beforeEach(async () => {
    ...
    repository = module.get<MockRepository<Student>>(
  ...
  describe('create', () => {
    ...
    it('should create and return a student DTO if studentId and email are unique', async () => {
      ...
      expect(repository.findOne).toHaveBeenCalledWith({
        where: [{ studentId: createDto.studentId }, { email: createDto.email }],
      });
    ...
    it('should throw ConflictException on save if studentId constraint violated (race condition)', async () => {
      ...
      const queryFailedError = new QueryFailedError('query', [], {
        code: '23505',
        message: 'students_studentId_key',
      } as any); // Adjust constraint name
      repository.save.mockRejectedValue(queryFailedError);
      await expect(service.create(createDto)).rejects.toThrow(
        new ConflictException(
          `Student ID "${createDto.studentId}" already exists.`,
        ),
      );
    });
  });
  ...
  describe('update', () => {
    ...
    it('should throw ConflictException if updated studentId already exists for another student', async () => {
      ...
      repository.findOne.mockResolvedValue({
        ...
        id: 'other-uuid',
        studentId: 'S-Other',
      }); // studentId exists on another student
      await expect(
        service.update('test-uuid', conflictingUpdateDto),
