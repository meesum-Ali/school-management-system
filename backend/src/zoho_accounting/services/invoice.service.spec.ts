...
describe('InvoiceService', () => {
  ...
  describe('createInvoice', () => {
    it('should create an invoice successfully', async () => {
      mockZohoClient.post.mockResolvedValue(
        mockAxiosResponse(mockZohoInvoiceResponse),
      );
    ...
    it('should throw error if zohoClient fails', async () => {
      ...
      await expect(service.createInvoice(mockInvoiceDto)).rejects.toThrow(
        'API Error',
      );
    });
  });
  ...
  describe('listInvoices', () => {
    const mockInvoicesListResponse: ZohoInvoicesListResponse = {
      ...
      invoices: [
        mockInvoiceEntity,
        { ...mockInvoiceEntity, invoice_id: 'inv-456' },
      ],
    };

    it('should list invoices successfully', async () => {
      mockZohoClient.get.mockResolvedValue(
        mockAxiosResponse(mockInvoicesListResponse),
      );
    ...
    it('should return empty array if no invoices found or unexpected response', async () => {
      mockZohoClient.get.mockResolvedValue(
        mockAxiosResponse({ code: 0, message: 'success', invoices: null }),
      );
  ...
  describe('updateInvoice', () => {
    ...
    const updatedInvoiceEntity: Invoice = {
      ...mockInvoiceEntity,
      notes: 'Updated notes',
    };
    const updatedResponse: ZohoInvoiceResponse = {
      ...mockZohoInvoiceResponse,
      invoice: updatedInvoiceEntity,
    };
  ...
  describe('deleteInvoice', () => {
    it('should delete an invoice successfully', async () => {
      mockZohoClient.delete.mockResolvedValue(
        mockAxiosResponse({ code: 0, message: 'Invoice deleted.' }),
      );
  ...
  describe('emailInvoice', () => {
    it('should send email for invoice successfully', async () => {
      mockZohoClient.post.mockResolvedValue(
        mockAxiosResponse({ code: 0, message: 'success' }),
      );
      const result = await service.emailInvoice('inv-123', {
        to_mail_ids: ['test@example.com'],
      });
      expect(result).toBe(true);
      expect(zohoClient.post).toHaveBeenCalledWith('invoices/inv-123/email', {
        to_mail_ids: ['test@example.com'],
      });
    });
    ...
    it('should return false if email sending was not clearly successful by message', async () => {
      mockZohoClient.post.mockResolvedValue(
        mockAxiosResponse({ code: 0, message: 'Email queued' }),
      );
      const result = await service.emailInvoice('inv-123');
    ...
    it('should return false if code is not 0', async () => {
      mockZohoClient.post.mockResolvedValue(
        mockAxiosResponse({ code: 1, message: 'failure' }),
      );
      const result = await service.emailInvoice('inv-123');
      expect(result).toBe(false);
    });
  ...
  describe('markAsSent', () => {
    it('should mark an invoice as sent successfully', async () => {
      mockZohoClient.post.mockResolvedValue(
        mockAxiosResponse(mockZohoInvoiceResponse),
      ); // Assuming it returns the updated invoice
    ...
    it('should fetch invoice if status change returns only success message', async () => {
      mockZohoClient.post.mockResolvedValue(
        mockAxiosResponse({ code: 0, message: 'Invoice status updated.' }),
      );
      mockZohoClient.get.mockResolvedValue(
        mockAxiosResponse(mockZohoInvoiceResponse),
      ); // For the subsequent getInvoice call
  ...
  describe('voidInvoice', () => {
    ...
    it('should fetch invoice if status change to void returns only success message', async () => {
      mockZohoClient.post.mockResolvedValue(
        mockAxiosResponse({
          code: 0,
          message: 'Invoice status updated to void.',
        }),
      );
