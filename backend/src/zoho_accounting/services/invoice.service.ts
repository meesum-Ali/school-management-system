...
export class InvoiceService {
  ...
  async createInvoice(invoiceDto: CreateInvoiceDto): Promise<Invoice> {
    ...
    try {
      const response: AxiosResponse<ZohoInvoiceResponse> = await this.zohoClient.post<ZohoInvoiceResponse>(
        this.endpoint,
        invoiceDto,
      );
  ...
  async getInvoice(invoiceId: string): Promise<Invoice | null> {
    ...
    try {
      const response: AxiosResponse<ZohoInvoiceResponse> = await this.zohoClient.get<ZohoInvoiceResponse>(
  ...
  async listInvoices(params?: Record<string, any>): Promise<Invoice[]> {
    ...
    try {
      const response: AxiosResponse<ZohoInvoicesListResponse> = await this.zohoClient.get<ZohoInvoicesListResponse>(
        this.endpoint,
        params,
      );
   ...
   * @throws HttpException if the API request fails or the response structure is unexpected.
   */
  async updateInvoice(invoiceId: string, updateDto: Partial<CreateInvoiceDto>): Promise<Invoice> {
    this.logger.log(`Attempting to update invoice with ID: ${invoiceId}`);
    try {
      const response: AxiosResponse<ZohoInvoiceResponse> = await this.zohoClient.put<ZohoInvoiceResponse>(
        `${this.endpoint}/${invoiceId}`,
        updateDto,
      );
      if (response.data && response.data.invoice) {
  ...
  async emailInvoice(invoiceId: string, emailDto?: any): Promise<boolean> {
    ...
    try {
      ...
      const response = await this.zohoClient.post<any>(`${this.endpoint}/${invoiceId}/email`, emailDto || {});
      if (response.data && response.data.code === 0 && response.data.message === 'success') { // Common Zoho success indication
  ...
  async markAsSent(invoiceId: string): Promise<Invoice> {
    ...
    try {
      const response = await this.zohoClient.post<ZohoInvoiceResponse>(`${this.endpoint}/${invoiceId}/status/sent`, {});
      if (response.data && response.data.invoice) {
        this.logger.log(`Invoice ${invoiceId} marked as sent.`);
        return response.data.invoice;
      } else if (response.data && response.data.code === 0) { // Some status changes might just return a success message
  ...
  async voidInvoice(invoiceId: string): Promise<Invoice> {
    ...
    try {
      const response = await this.zohoClient.post<ZohoInvoiceResponse>(`${this.endpoint}/${invoiceId}/status/void`, {});
       if (response.data && response.data.invoice) {
